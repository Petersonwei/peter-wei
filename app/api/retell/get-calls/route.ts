...
import type { GetCallsResponse, TransformedCall, CustomAnalysisData } from '@/types/retell';
      ...
      callsResponse.map(async (call) => {
        try {
          ...
          const transformedCall: TransformedCall = {
            ...
            duration: 'duration' in call ? (call.duration as number) : undefined,
            created_at: 'created_at' in call && typeof call.created_at === 'string' ? call.created_at : new Date().toISOString(),
            analysis: {
